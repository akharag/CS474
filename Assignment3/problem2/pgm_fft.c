...
void fft_2d(unsigned long int n, unsigned long int m, float *real_fuv, float *img_fuv, int isign)
{
}
...
struct pgm_complex * pgm_complex_new(struct pgm_image *image)
{
    ...
    {
        comp_img->data[q] = (double)image->data[q] + 0.0 * I;
       // printf("%llu \n", q);
    }
    return comp_img;
}
...
char pgm_complex_decompose(struct pgm_complex *c_image, struct pgm_image *r_image, struct pgm_image *i_image)
{
    ...
    unsigned long long total = c_image->n * c_image->m;
    double r_greatest=1.0;
    double i_greatest=1.0;
    for (unsigned long long q = 0; q < total; q++)
    {
        double r_val = log(1+cabs(c_image->data[q]));
        double i_val = log(1+carg(c_image->data[q]));
        if (r_val > r_greatest) r_greatest = r_val;
        if (i_val > i_greatest) i_greatest = i_val;
    }
    double r_scale=r_image->limit / r_greatest;
    double i_scale=i_image->limit / i_greatest;
    for (unsigned long long q = 0; q < total; q++)
    {
        r_image->data[q] = (char) (r_scale*log(1+cabs(c_image->data[q])));
        i_image->data[q] = (char) (i_scale*log(1+carg(c_image->data[q])));
    }
    return 1;
}
